/*
  ...
  */ 
  ...
  2;e=c('<div id="spinner-'+c.fn.spinner.count+'" class="spinner" />').css({position:"absolute","z-index":999,left:"50%",top:"50%",margin:d*-1+"px 0 0 "+d*-1+"px",width:a.sqr,height:a.sqr});$canv=c("<canvas />").attr({width:a.sqr,height:a.sqr});g=f.css("position");log(g);g==="static"&&f.css({position:"relative"});$canv.appendTo(e);e.appendTo(f);if($canv[0].getContext){b=$canv[0].getContext("2d");b.translate(d,d);b.lineWidth=Math.ceil(a.sqr*0.025);b.lineCap="round";this.loop=setInterval(i,1E3/a.framerate)}else{$canv.remove();
